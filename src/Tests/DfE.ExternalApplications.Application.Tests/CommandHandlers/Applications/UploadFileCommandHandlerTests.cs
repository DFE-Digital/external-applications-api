...
public class UploadFileCommandHandlerTests
{
        ...
        Stream fileContent)
    {
        ...
        var uploadQueryable = new List<File> { existingFile }.AsQueryable().BuildMock();
        _uploadRepository.Query().Returns(uploadQueryable);
        ...
        Stream fileContent)
    {
        ...
        _fileStorageService.UploadAsync(Arg.Any<string>(), Arg.Any<Stream>(), Arg.Any<CancellationToken>())
